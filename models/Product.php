...
class Product
{
    ...
    public static function find($count = self::SHOW_BY_DEFAULT, $text = self::SHOW)
    {
        ...
        if (isset($_POST['submitt'])) { 
            $lowe = $_POST['low'];
            $highte = $_POST['hight'];
            $sql = 'SELECT id, name, price, is_new FROM product WHERE status = "1" AND price BETWEEN "'.$lowe.'" and "'.$highte.'" ';
        }
        elseif (isset($_POST['submit'])) { 
            $userName = $_POST['findtxt'];
            $sql = 'SELECT id, name, price, is_new FROM product WHERE status = "1" AND name LIKE "'.$userName.'%" ';
        }
        else {       
        // Текст запроса к БД
        $sql = 'SELECT id, name, price, is_new FROM product WHERE name LIKE "'.$text.'" ';
        }
